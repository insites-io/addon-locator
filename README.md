...
Google Maps is lazy-loaded on first user interaction (scroll / touchstart / click / keydown / mousemove) or `requestIdleCallback`, on both mobile and desktop — see [results.liquid](modules/locator/public/views/partials/website/results.liquid).
...
| `views/partials/website/list.liquid` | Directory layout — search bar, list panel, map panel |
| `views/partials/website/filters.liquid` | Search bar and controls bar |
| `views/partials/website/results.liquid` | Card list, map panel, Google Maps script |
| `views/partials/website/card.liquid` | Single result card (rendered by both SSR and AJAX) |
...
1. `views/partials/website/details.liquid` runs `get_location_detail.graphql` with `slug: context.params.slug2`.
...
| File | Purpose |
|---|---|
| `views/partials/website/details.liquid` | Profile partial — renders banner, contact card, and sanitised long description |
